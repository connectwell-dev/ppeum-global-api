...
export class AdminAuthService {
  ...
  async login(dto: LoginReqDto, req: Request): Promise<LoginResDto> {
    ...
    try {
      const employee = await this.prisma.employee.findFirst({
        ...
        select: { id: true, loginId: true, name: true, password: true, employmentStatus: true, adminPermission: true, userPermission: true },
      });
      ...
      if (!isPasswordValid) {
        await saveLoginHistory(this.prisma, employee.id, Platform.Admin, false, connectionInfo, '비밀번호 불일치');
        throw new CustomException('auth.invalid_credentials', 'BAD_REQUEST');
      }

      const sessionId = randomUUID();
      const accessTokenPayload = { id: employee.id, loginId: employee.loginId, name: employee.name, platform: Platform.Admin, sessionId, permission: employee.adminPermission };
      const refreshTokenPayload = { id: employee.id, platform: Platform.Admin, sessionId };
      ...
      await saveLoginHistory(this.prisma, employee.id, Platform.Admin, true, connectionInfo);
  ...
  async refresh(refreshToken: string): Promise<{ accessToken: string }> {
    try {
      ...
      try {
        ...
        if (payload.exp < Date.now() / 1000) throw new CustomException('auth.expired_token', 'UNAUTHORIZED');// 만료 체크
        if (payload.platform !== Platform.Admin) throw new CustomException('common.invalid_request', 'UNAUTHORIZED');
      } catch {
      ...
      const employee = await this.prisma.employee.findFirst({
        ...
        select: { id: true, loginId: true, name: true, adminPermission: true, userPermission: true },
      });
      if (!employee) throw new CustomException('auth.invalid_token', 'UNAUTHORIZED');

      const accessToken = this.jwtService.sign(
        { id: employee.id, loginId: employee.loginId, name: employee.name, platform: payload.platform, sessionId: payload.sessionId, permission: employee.adminPermission } as any,
  ...
  // - adminPermission 노출 (user-auth 와 반대)
  // ───────────────────────────────────────────────────────────────────────────
  async getMe(employeeId: number): Promise<GetAdminMeResDto> {
    try {
      const employee = await this.prisma.employee.findFirst({
      ...
      return {
        ...
        permission: (employee.adminPermission ?? {}) as Record<string, unknown>,
      };
    } catch (error) {
      if (error instanceof CustomException) throw error;
      throw new CustomException('common.errorMessage', 'INTERNAL_SERVER_ERROR');
